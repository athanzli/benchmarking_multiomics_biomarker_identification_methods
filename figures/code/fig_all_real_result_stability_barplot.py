...
MODEL_ORDER_EMPTY_FILLED = ['MOGLAM',
                            'Pathformer',
                            'DeePathNet',
                            'DeepKEGG',
                            'TMONet',
...
PERF_MODEL_ORDER_EMPTY_FILLED = None
...
# extended mods
###############################################################################
bkacc_metrics = ['RBO','RPSD','KendallTau']

################################
data_plots_surv = []
data_plots_drug = []
for i in range(len(bkacc_metrics)):
    data_plot = pd.read_csv(f"../../result/result_TCGA_bk_stability_{bkacc_metrics[i]}.csv", index_col=0) # TODO
    data_plot = data_plot.loc[MODELS]
    
    tmp = data_plot.loc[:, data_plot.columns.str.contains('surv')]
    tmp.columns = [bkacc_metrics[i] + '-' + col[:] for col in tmp.columns]
...
data_plot_m1 = data_plot.loc[MODELS, data_plot.columns.str.contains('RBO-')].copy()
data_plot_m2 = data_plot.loc[MODELS, data_plot.columns.str.contains('RPSD-')].copy()
data_plot_m3 = data_plot.loc[MODELS, data_plot.columns.str.contains('KendallTau-')].copy()
...
create_bar_plot(data_plot=data_plot_m1, cmap_name="Purples_r",
                save_name="fig_all_real_stability_RBO_barplot")
create_bar_plot(data_plot=data_plot_m2, cmap_name="Blues", # NOTE not reversed.
                save_name="fig_all_real_stability_RPSD_barplot")
...
create_bar_plot(data_plot=data_plot_m3, cmap_name=custom_cmap,
                save_name="fig_all_real_stability_KendallTau_barplot")
