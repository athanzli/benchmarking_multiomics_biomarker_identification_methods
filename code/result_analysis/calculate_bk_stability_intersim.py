...
res_all = pd.DataFrame(
    columns = ['model','base_name','metric','value']
)
        ...
        for fold in FOLDS:
            ...
            if model in MODELS_WITH_BARE_GENES_FT_SCORES:
                ...
                if model in MODELS_GENE_CENTRIC:
                    ...
                    mmdic = mod_mol_dict(ft.index)
                    ft.index = mmdic['mols']
                    ft = ft.groupby(ft.index).mean() # NOTE
            ...
            # if len(set(bk) - set(ft_gset)) > 0:
            #     print(f"model={model}, effect={effect}, task={task}, #genes in BK set but not in ft gset: ", len(set(bk) - set(ft_gset)))
            ...
            if fold == len(FOLDS) - 1:
                if not (np.all([(len(rankings[0]) == len(rankings[i])) for i in range(len(rankings))])):
                    assert model == 'ttest'
                    tmp = rankings[0]
                    for i in range(len(rankings)):
                        tmp = np.intersect1d(tmp, rankings[i])
                    for i in range(len(rankings)):
                        rankings[i] = rankings[i][np.isin(rankings[i], tmp)]

                res_all = pd.concat([res_all, pd.DataFrame({
                    'model': [model],
                    'base_name': [base_name],
                    'metric': ['KendallTau'],
                    'value': average_kendall_tau(rankings)
                })], ignore_index=True)
                res_all = pd.concat([res_all, pd.DataFrame({
                    'model': [model],
                    'base_name': [base_name],
                    'metric': ['RBO'],
                    'value': average_rbo(rankings, p=0.98)
                })], ignore_index=True)
                res_all = pd.concat([res_all, pd.DataFrame({
                    'model': [model],
                    'base_name': [base_name],
                    'metric': ['RPSD'],
                    'value': percentile_standard_deviation(rankings, bk)
                })], ignore_index=True)

#%%
res_all.to_csv("../../result/bkstability_res_InterSIM.csv")
