...
class GraphLearningLayer(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Tensor):
        ...
        distance = 1 - torch.mul(x_i, x_j) / ((m1 * m2) + epsilon)
        ...
        gl_loss = None
        if self.training:
            gl_loss = self._graph_learning_loss(x_hat, soft_adj)
    ...
    def _graph_learning_loss(self, x_hat: Tensor, adj: Tensor):
        ...
        dist_loss = adj + self.eta * torch.norm(1 - torch.mul(x_i, x_j) / ((m1 * m2) + epsilon), dim=2)
        dist_loss = torch.exp(dist_loss)

        dist_loss = torch.sum(dist_loss, dim=(0, 1)) / (N * N)

        f_norm = torch.norm(adj, dim=(0, 1))
...
def init_optim(num_view, model_dict, lr_c=1e-5, lr_e=1e-3):
    ...
    gcn_e_parameters = []
    for i in range(num_view):
        optim_dict["C{:}".format(i + 1)] = torch.optim.AdamW(list(model_dict["GL{:}".format(i + 1)].parameters()) +
                                                             list(model_dict["E{:}".format(i + 1)].parameters()),
                                                             lr=lr_e)
    if num_view >= 2:
        optim_dict["H"] = torch.optim.AdamW(list(model_dict["H"].parameters()) + 
