...
def classification_loss(loss_name, y_true, y_pred ,reduction='mean', class_weights=None):
    """
    ...
    """
    if loss_name == 'BCE':
        return nn.BCEWithLogitsLoss(reduction=reduction, weight=class_weights)(y_true, y_pred)
    elif loss_name == 'CE':
        return nn.CrossEntropyLoss(reduction=reduction, weight=class_weights)(y_true, y_pred)
    else:
