...
class CancerDataset_forbk(Dataset):
    def __init__(
        self,
        omics,
        clinical_data,
        cancer_fact_mapping):
        r"""
        ID column of omics and clinical_data must be the same. Both are sample IDs.
        Inputs are either training data or testing data.
        
        Args:
            omics (dict): dictionary of omics dataframes
        ...
        self.cancer_fact_mapping = cancer_fact_mapping

    def __len__(self):
        return len(self.clinical_data)

    def __getitem__(self, idx):
        sample_id = self.clinical_data['ID'].iloc[idx]
        ...
        OS_time = torch.Tensor(sample_clinical_data['OS.time'].values) / 30
        cancer = sample_clinical_data['Cancer_Type'].values.tolist()
        cancer_type = torch.LongTensor([self.cancer_fact_mapping[cancer[0]]])
