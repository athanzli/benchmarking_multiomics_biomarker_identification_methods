...
def reparameterize(mean, logvar):
    logvar = torch.clamp(logvar, min=-MAX_LV, max=MAX_LV) # NOTE to avoid nan
...
def product_of_experts(mu_set_, log_var_set_):
    log_var_set_ = [torch.clamp(lv, min=-MAX_LV, max=MAX_LV) for lv in log_var_set_] # NOTE to avoid nan

    tmp = 0
    for i in range(len(mu_set_)):
        tmp += torch.div(1, torch.exp(log_var_set_[i]) + EPSILON)

    poe_var = torch.div(1., tmp + EPSILON)
    poe_log_var = torch.log(poe_var)

    tmp = 0.
    for i in range(len(mu_set_)):
        tmp += torch.div(1., torch.exp(log_var_set_[i]) + EPSILON) * mu_set_[i]
    poe_mu = poe_var * tmp
...
class DownStream_predictor(nn.Module):
    def __init__(self, modal_num, modal_dim, latent_dim, encoder_hidden_dim, decoder_hidden_dim, 
        # pretrain_model_path, # NOTE
        pretrain_model_dict,
        ...
        if len(pretrain_model_dict) > 0:
            print('load pretrain model')
            self.cross_encoders.load_state_dict(pretrain_model_dict)
    ...
    # NOTE
    def forward_for_ig(self, input_x):
        r"""
        
        Args:
            input_x: dict of tensor, input data
        """
        ks, vs = list(input_x.keys()), list(input_x.values())
        batch_size = vs[0].shape[0]
        mods = np.unique(ks)
        omics = {mod: i for i, mod in enumerate(mods)}

        multi_representation = self.cross_encoders.get_embedding(input_x, batch_size=batch_size, omics=omics)
        downstream_output = self.downstream_predictor(multi_representation)
