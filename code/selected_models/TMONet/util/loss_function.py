...
def KL_loss(mu, logvar, beta, c=0.0):
    logvar = torch.clamp(logvar, min=-MAX_LV, max=MAX_LV) 
    KLD_1 = -0.5 * torch.sum(1 + logvar - mu.pow(2) - torch.exp(logvar))
    return beta * KLD_1


def KL_divergence(mu1, mu2, log_sigma1, log_sigma2):
    log_sigma1 = torch.clamp(log_sigma1, min=-MAX_LV, max=MAX_LV) 
    log_sigma2 = torch.clamp(log_sigma2, min=-MAX_LV, max=MAX_LV) 
...
def cox_loss(survtime, censor, hazard_pred):
    ...
    theta = torch.clamp(theta, min=-MAX_LV, max=MAX_LV) 
