...
class GraphLearn(nn.Module):
    ...
    def forward(self,x):
        initial_x = x.clone() # (N, D)
        num, feat_dim = x.size(0), x.size(1) # N; D
        if self.mode == 'adaptive-learning':
            x = x.repeat_interleave(num, dim=0) # NOTE creates an extremely large tensor of size (N, N, feat_dim)
            ...
            output = gen_adj_mat_tensor(x_norm, adj_parameter_adaptive, self.adj_metric, device=x.device) # NOTE added device
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_data_dims, num_class):
...
def init_model_dict(num_view, num_class, dim_list, dim_he_list, input_data_dims, adj_parameter, mode,featuresSelect_list,gcn_dopout=0.5): 
    model_dict = {}
    ...
    if num_view >= 2:
        ...
        model_dict["OIRL"] = TransformerEncoder(input_data_dims, num_class) 
    return model_dict
