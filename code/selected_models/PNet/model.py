...
from .model_layer import Diagonal, SparseTF
...
class pnet_Sparselayer(nn.Module):
    def __init__(self, mapp,num_classes=2,dropout_rate=0,kernel_initializer='glorot_uniform', activation='tanh', use_bias=True,
        ...
        self.SparseTF_layer = SparseTF(units=self.n_pathways,input_shape=self.n_genes,map=self.mapp, 
                                       nonzero_ind=None,
        ...
        if self.n_hidden_layers > 0:
            ...
            self.layers = nn.ModuleList([
                pnet_Sparselayer(
                    ...
                    dropout_rate=self.dropout[i+1],
                    kernel_initializer=self.kernel_initializer,
                    activation=self.activation,
                    use_bias=self.use_bias,
                    bias_initializer=self.bias_initializer) for i in range(len(maps[0:-1]))]) #maps[0:-1]
