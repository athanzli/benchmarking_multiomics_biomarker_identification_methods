...
class GraphCNN(nn.Module):
    ...
    def __preprocess_neighbors_maxpool(self, batch_graph):
        ...
        # return torch.LongTensor(padded_neighbor_list) # NOTE
    ...
    def __preprocess_neighbors_sumavepool(self, batch_graph):
        ...
        # Adj_block_elem = torch.ones(Adj_block_idx.shape[1]) # NOTE
        ...
        if not self.learn_eps:
            ...
            self_loop_edge = torch.LongTensor([list(range(num_node)), list(range(num_node))]).to(self.device)
            elem = torch.ones(num_node, device=self.device)

            Adj_block_idx = torch.cat([Adj_block_idx, self_loop_edge], 1)
            Adj_block_elem = torch.cat([Adj_block_elem, elem], 0)

        # Adj_block = torch.sparse.FloatTensor(Adj_block_idx, Adj_block_elem, torch.Size([start_idx[-1],start_idx[-1]])) #NOTE
        Adj_block = torch.sparse.FloatTensor(Adj_block_idx.to(self.device),
                                            Adj_block_elem,
                                            torch.Size([start_idx[-1], start_idx[-1]])).to(self.device)
    ...
    def __preprocess_graphpool(self, batch_graph):
        ...
        elem = torch.FloatTensor(elem).to(self.device)
        idx = torch.LongTensor(idx).to(self.device).transpose(0,1)
    ...
    def next_layer_eps(self, h, layer, padded_neighbor_list = None, Adj_block = None):
        ...
        if self.neighbor_pooling_type == "max":
            ...
            if self.neighbor_pooling_type == "average":
                #If average pooling
                degree = torch.spmm(Adj_block, torch.ones((Adj_block.shape[0], 1), device=self.device)) # NOTE added device
