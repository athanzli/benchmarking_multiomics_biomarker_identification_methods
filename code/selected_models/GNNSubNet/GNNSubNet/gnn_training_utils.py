...
def pass_data_iteratively(model, graphs, minibatch_size = 32):
    ...
    for i in range(0, len(graphs), minibatch_size):
        ...
        # # Move each graph's tensors to the model's device
        # for graph in batch_graph:
        #     if graph.node_features is not None:
        #         graph.node_features = graph.node_features.to(model.device)
        #     if graph.edge_mat is not None:
        #         graph.edge_mat = graph.edge_mat.to(model.device)
        output.append(model(batch_graph).detach())
    return torch.cat(output, 0)
