...
from .features_computation import gen_data_community
...
def convert_to_s2vgraph(graphs, device):
    s2v_graphs = []
    for graph in graphs:
        edge_index = graph.edge_index
        ...
        if edge_list:
            new_edge_index = torch.tensor(np.array(edge_list).T, dtype=torch.long)
        else:
        ...
        max_neighbor = max(degrees) if degrees else 0
        s2v_graphs.append(S2VGraph(g, graph.y.item(), None, graph.x, new_edge_index, max_neighbor, neighbors, device))
    return s2v_graphs
