...
def make_image(id, met, all_genes, img_size=193):
    ...
    names = all_genes['name2'].values
    chrs = all_genes['chr'].values
    
    for cnt, (i, j) in enumerate(np.ndindex(img_size + 1, img_size + 1)):
        if cnt < genes_count:
            name_val = names[cnt]
            img = ImageCell(name_val, mod1_val=None, mod2_val=None, mod3_val=None, mod4_val=None, chr=chrs[cnt])
            cell_dict[name_val] = img
        else:
            img = ImageCell(None, None, None, None, None, None)
        img.i, img.j = i, j
    return Image(id=id, met=met, dict_of_cells=cell_dict)
